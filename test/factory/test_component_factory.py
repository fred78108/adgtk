...
Testing
=======
python -m pytest -s test/factory/test_component_factory.py
...
def test_factory_can_init_behavior():
    ...
    assert result.foo == 42     # type: ignore
...
def test_register_basic_callable():
    fid = component.register(
        dummy_creator, group="dummies", tags=["alpha"], summary="basic test")
...
def test_register_duplicate_id_raises():
    _ = component.register(dummy_creator, group="dups", factory_id="myid")
...
def test_list_entries_all():
    id1 = component.register(
        dummy_creator, group="x", tags=["t1"], summary="s1")
    id2 = component.register(
        dummy_creator, group="y", tags=["t2"], summary="s2")
...
def test_list_entries_group_and_tags():
    component.register(
        dummy_creator, group="g", tags=["a", "b"], summary="one")
