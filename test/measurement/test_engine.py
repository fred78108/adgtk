...
This module contains unit tests for the `MeasurementEngine` class and its
methods. The tests ensure that the engine behaves as expected under various
scenarios.
...
def test_supports_measurement_type_mixed_annotations():
    """Test `supports_measurement_type` with a function that has mixed
    annotations."""
...
def test_add_measurement_invalid_id(mock_measurement_engine):
    ...
    with patch("adgtk.measurements.engine.get_measurement_factory_entry",
...
def test_measure(mock_measurement_engine):
    ...
    assert mock_measurement_engine.metric_tracker.get_latest_value(
        "test_id") == 12
...
def test_report_with_multiple_measurement_rounds(mock_measurement_engine):
    ...
    assert len(report["measurements"]) == 2
    for measurement in report["measurements"]:
        assert len(measurement["data"]) >= 2  # At least 2 measurements per
